	...
	var	document = window.document,
			...
			setAnimation: (obj)=> {
				...
				return (
					(animObj, animPath)=> {
						...
						var setPixelVal = (coord)=> coord + "px"
						...
						var show = ()=> target.style.visibility = "visible";


						var anim = (index)=>setCoord(path[index]);
		...
		normalizeAPI = ()=> {
			...
			if(document.getElementById) {
				getFn = (id)=> document.getElementById(id);


			} else {
				if(document.all) {
					getFn = (id)=> document.all[id];
				}
			}
			DMJS.get = getFn;
			if(document.addEventListener) {
				...
				removeEventFn = (el, type, fn)=> 	el.removeEventListener(type, fn, false);
				...
				if(document.attachEvent) {
					...
					removeEventFn = (el, type, fn)=>el.detachEvent("on" + type, fn);
